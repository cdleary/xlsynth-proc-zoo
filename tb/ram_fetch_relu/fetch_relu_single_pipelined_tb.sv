...
module tb;
  ...
  initial begin
    ...
    if ($test$plusargs("dump_vcd")) begin
      if (!$value$plusargs("dump_path=%s", dump_path)) begin
        dump_path = "fetch_relu_single_pipelined_tb.vcd";
      end
      $dumpfile(dump_path);
      $dumpvars(0, tb);
    end
  ...
  __fetch_relu_single_pipelined__FetchReluSinglePipelined_0_next dut (
      .clk(clk),
      .rst(rst),
      .fetch_relu_single_pipelined__addr_in(addr_in),
      .fetch_relu_single_pipelined__addr_in_vld(addr_in_vld),
      .fetch_relu_single_pipelined__ram_req_rdy(ram_req_rdy),
      .fetch_relu_single_pipelined__ram_resp(ram_resp),
      .fetch_relu_single_pipelined__ram_resp_vld(ram_resp_vld),
      .fetch_relu_single_pipelined__out_ch_rdy(out_ch_rdy),
      .fetch_relu_single_pipelined__addr_in_rdy(addr_in_rdy),
      .fetch_relu_single_pipelined__ram_req(ram_req),
      .fetch_relu_single_pipelined__ram_req_vld(ram_req_vld),
      .fetch_relu_single_pipelined__ram_resp_rdy(ram_resp_rdy),
      .fetch_relu_single_pipelined__out_ch(out_ch),
      .fetch_relu_single_pipelined__out_ch_vld(out_ch_vld)
  );
